...
M_FIT <- readRDS("diversification/pgls-model-fits.rds")
M_FIT_bamm <- readRDS("diversification/pgls-model-fits_bamm.rds")
...
pdf(paste0('~/Downloads/figS24.pdf'), width = 3, height = 5)
...
# rect(ytop = colubriformBounds[1] - 0.5, ybottom = colubriformBounds[2] + 2, xleft = 215, xright = int[length(int)] + refW+2, xpd = NA, lwd = 0.5, lty = 3)


text(x = int[length(int)] + refW+5, y = 70, 'Snakes', pos = 4, xpd = NA, cex = 0.75, srt = 270)
# text(x = grconvertX(0.80, from = 'nfc', to ='user'), y = int[length(int)] + refW + 30, 'Serpentes', pos = 4, xpd = NA, cex = 0.5)
...
#pdf('~/Downloads/speciation_violinPlots.pdf', width = 7, height = 7)

par(mar = c(5,0,2,0))

plot.new()
#plot.window(xlim = c(0.5, length(clades)), ylim = range(log(alldat$bamm), na.rm = TRUE))
plot.window(xlim = c(0.5, length(clades)), ylim = c(-3.5, 0))
...
# text(x = 1:length(clades) + 0.6, y = -4.7, cladeLabels, srt = 50, pos = 2, xpd = NA, cex = 0.5, font = fontvec)
text(x = 1:6 + 0.6, y = -3.8, cladeLabels[1:6], srt = 50, pos = 2, xpd = NA, cex = 0.5, font = fontvec[1:6])

text(x = 7 + 0.55, y = -3.8, 'Australian', srt = 50, pos = 2, xpd = NA, cex = 0.5, font = fontvec[7])
text(x = 7 + 0.95, y = -3.8, 'sphenomorphines', srt = 50, pos = 2, xpd = NA, cex = 0.5, font = fontvec[7])

text(x = 8 + 0.55, y = -3.8, 'limb-reduced', srt = 50, pos = 2, xpd = NA, cex = 0.5, font = fontvec[8])
text(x = 8 + 0.95, y = -3.8, 'non-snakes', srt = 50, pos = 2, xpd = NA, cex = 0.5, font = fontvec[8])
...
for (i in 1:length(clades)) {
	...
	xx <- log(setNames(alldat[, 'bamm'], alldat$treename)[sp])
...
dat <- alldat[, c('elongationIndex', 'bamm')]
...
# cols <- rep('slateblue1', nrow(dat))
# cols[rownames(dat) %in% snakes] <- 'coral2'
# table(cols)
# points(dat, bg = adjustcolor(cols, alpha.f = 0.75), lwd = 0.5, pch = 21)
...
r2vals <- sapply(M_FIT_bamm, function(x) summary(x)$adj.r.squared)
...
M_FIT2 <- M_FIT_bamm[order(r2vals, decreasing = TRUE)]
